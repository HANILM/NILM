...
# 从配置字典中提取参数
mode = config['mode']
closed_test = config['closed_test']
ddpg_test = config['ddpg_test']
...
if __name__ == '__main__':
    ...
    if mode=='closed_train':
        ...
        if closed_test == True:  # if true begin testing
        ...
        train_main, train_appliances, test_main, test_appliances =load_data(train_start,train_end,test_start,test_end,building,meter_key1,window_size,batch_size,method='ddpg_train',sample_period=6)
        ...
        directory = './exp' + script_name + mode + "head" + str(head) + "model" + str(
            model) + "layer" + str(
            layer) + './'
