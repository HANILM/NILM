...
closed_learning_rate = config['closed_learning_rate']
epoch = config['epoch']
tau = config['tau']
...
learning_starts = config['learning-starts']
print_log = config['print_log']
update_iteration = config['update_iteration']
policy_frequency = config['policy-frequency']
model = config['model']
layer = config['layer']
head = config['head']
...
state_dim=config['window_size']

d_model = config['model']
n_layers = config['layer']
d_k = d_v = config['model']
...
parser.add_argument('--mode', default='ddpg_train', type=str)  # mode = 'closed_train' or 'ddpg_train'
...
if __name__ == '__main__':
    epochs = epoch
    train_start, train_end = "2013-6-15", "2013-7-06" # Select training and testing date
    ...
    if args.mode=='closed_train':
        ...
        learning_rate=closed_learning_rate
        ...
        train_main, train_appliances, test_main, test_appliances,appliance_means,appliance_std =load_data(train_start,train_end,test_start,test_end,building,meter_keys,window_size,batch_size,method='ddpg_train',sample_period=6)
        ...
        env = Enviroment(train_main, train_appliances)

        # directory is the path of running log
        directory = './exp' + script_name + args.mode + "head" + str(head) + "model" + str(
            model) + "layer" + str(
            layer) + './'

        agent = DDPG(model, window_size,layer,action_dim,  directory,batch_size,tau,gamma,
                     policy_frequency,log_interval,d_k,d_v,head,d_ff,capacity)
        ...
        if args.ddpg_test == True:
            ...
            appliance1_power = appliance1_power * appliance_std + appliance_means

            test_appliance1 = test_appliances * appliance_std + appliance_means

            valid_predictions1 = appliance1_power.flatten()

            appliance1_power = np.where(valid_predictions1 > 0, valid_predictions1, 0).squeeze()
        ...
        for i in range(max_episode):
            ...
            total_reward += reward
            print(
                "max_episode=%d i=%d Total Reward=%f................................" % (
                    max_episode, i, total_reward))
